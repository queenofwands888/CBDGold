...
const GovernancePanel: React.FC = () => {
  ...
  const vote = async (id: number) => {
    ...
    try {
      await voteProposal(id, proposal.weedRequired, proposal.title);
    } finally {
      setVotingId(null);
    }
  };
            ...
            <div key={p.id} className="glass-card rounded-lg p-4">
              ...
              <div className="flex justify-between items-center text-xs">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => vote(p.id)}
                    disabled={accountAssets.weed < p.weedRequired || votingId === p.id}
                    className={`px-3 py-1 rounded-full text-xs font-semibold ${accountAssets.weed >= p.weedRequired && votingId !== p.id ? 'bg-purple-600 text-white hover:bg-purple-700' : 'bg-gray-600 text-gray-400 cursor-not-allowed'}`}
                  >
