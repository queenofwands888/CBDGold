...
import VapesSection from './VapesSection';
...
const DashboardPanel: React.FC = () => {
  ...
  // Select featured products (first 3)
  const featuredProducts = CBD_VAPES.slice(0, 4).map((v: any) => ({
    ...v,
    prices: { algo: v.priceAlgo, usdc: v.priceUsdc, hemp: v.hempEarned },
    hempEarned: v.hempEarned
  }));

  // Dummy token prices for featured section
  const tokenPrices = { HEMP: 0.00025, ALGO: 1, USDC: 1 };
  // Removed unused variable oracleMeta
  ...
  return (
    <div className="space-y-8" data-testid="dashboard-root">
      ...
      <div className="mt-10">
        <h2 className="text-2xl font-bold text-yellow-300 mb-4 text-center tracking-wide drop-shadow-lg">Featured Products</h2>
