...
async function runProductsFlood() {
  ...
  console.log(`Products flood: Success=${success}, RateLimited=${rateLimited}, Errors=${errors}`);
}

async function runInvalidPayloadTest() {
  try {
    const res = await axios.post(`${API_URL}/api/products`, {});
    console.log('Unexpected POST /api/products response:', res.data);
  } catch (err) {
